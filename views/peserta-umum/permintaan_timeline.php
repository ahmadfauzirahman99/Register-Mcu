...
?>
<div class="progress">
    <a href="<?php echo Url::to(['daftar']); ?>" class="<?php echo Yii::$app->controller->action->id=="daftar" ? ( $user!=NULL ? ( $user['u_biodata_finish_at']==NULL ? 'inactive' : 'active' ) : 'inactive' ) : "" ?> progress-bar <?php echo $user!=NULL ? ( $user['u_biodata_finish_at']==NULL ? 'progress-bar-warning' : 'progress-bar-success' ) : 'progress-bar-warning' ?> progress-daftar" style="width: <?php echo $width; ?>%" title="<?php echo $user!=NULL ? ($user['u_biodata_finish_at']!=NULL ? 'selesai' : 'belum selesai') : '' ?>">
   ...
   if($user!=NULL){
        if($user['u_jenis_mcu_id']==1){
            ?>
            <a href="<?php echo Url::to(['kuisioner-preemployee']); ?>" class="<?php echo Yii::$app->controller->action->id=="kuisioner-preemployee" ? ( $user!=NULL ? ($user['u_kuisioner3_finish_at']==NULL ? 'inactive' : 'active') : 'inactive' ) : "" ?> progress-bar <?php echo $user!=NULL ? ($user['u_kuisioner3_finish_at']==NULL ? 'progress-bar-warning' : 'progress-bar-success') : 'progress-bar-warning' ?> progress-kuisioner3" style="width: <?php echo $width; ?>%" title="<?php echo $user!=NULL ? ($user['u_kuisioner3_finish_at']!=NULL ? 'selesai' : 'belum selesai') : '' ?>">
